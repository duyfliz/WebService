...
/*		// SELECT name FROM Student
	...
	public void test2() throws JSQLParserException {
		...
		Column id1 = new Column();
		id1.setColumnName("CustomerID");
		id1.setTable(orders);
		
		Column id2 = new Column();
		id2.setColumnName("CustomerID");
		id2.setTable(customers);
		
		Column id3 = new Column();
		id3.setColumnName("OrderID");
		id3.setTable(orders);
		
		LongValue val = new LongValue(10);
		
		SelectExpressionItem item = new SelectExpressionItem();
		Column all = new Column();
		all.setColumnName("*");
		item.setExpression(all);
		
		EqualsTo equal = new EqualsTo();
		equal.setLeftExpression(id1);
		equal.setRightExpression(id2);
		
		Join join = new Join();
		join.setRightItem(customers);
		join.setOnExpression(equal);
		join.setSimple(true);
		
		List<Join> joins = new ArrayList<Join>();
		joins.add(join);
		
		MinorThan minor = new MinorThan();
		minor.setLeftExpression(id3);
		minor.setRightExpression(val);
		
		
		test.addSelectItems(item);
		test.setFromItem(orders);
		test.setJoins(joins);
		test.setWhere(minor);
